...
import { DEFAULT_TEST_DURATION } from "../constants/testConfig";
import { getRandomPassage } from "../data/passages";
import { calculateAccuracy } from "../utils/calculateAccuracy";
import { calculateWPM } from "../utils/calculateWPM";
import { countCharacters } from "../utils/typingUtils";
import useTimer from "./useTimer";

export default function useTypingTest() {
  const [duration, setDuration] = useState(DEFAULT_TEST_DURATION);
  ...
  }, [typedText, passage, finishTest]);

  const { correct, incorrect } = useMemo(() => {
    return countCharacters(typedText, passage);
  }, [typedText, passage]);

  const wpm = useMemo(() => {
    return calculateWPM(correct, elapsedTime);
  }, [correct, elapsedTime]);
